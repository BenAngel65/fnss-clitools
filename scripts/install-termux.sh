...
UPDATE_DST="$HOME/.local/bin/fnssclitools-update"
if [ -f "$UPDATE_SRC" ]; then
    mkdir -p "$(dirname "$UPDATE_DST")"
    cp "$UPDATE_SRC" "$UPDATE_DST"
    chmod +x "$UPDATE_DST"
    # Termux 上 /usr/bin/env 不存在（或指向 toybox），shebang 必须用绝对路径
    sed -i "1s|.*|#!/data/data/com.termux/files/usr/bin/bash|" "$UPDATE_DST"
...
echo "  安装命令：oinbox, odiary, onote, fnsssync, fnssclitools-update"
echo "============================================"
echo ""
echo "下一步："
